...
| `protocol`                                                                                      | everything `--json` prints and nothing that touches a tree: intents, `Match`/`MatchId`/`Occurrence` with `Confidence` and `Reason`, `Selection`, the answers (`Outline`, `Deps`, `Explanation`, `References`, `Locations`, `File`), `Notice`, `Respelling`, `Reach`, `Template`, one result per command, `HistoryEntry`, `FileChange` with a `UnifiedDiff` (`similar`), `SCHEMA` and the `Response` envelope, `Request`/`Answer` (every command as one value and every result as one), `Call`/`Reply` (a request with an `id` and its reply, what `vvv serve` speaks), `Failure` with its `ErrorCode` (what an error is on the wire; `EngineError::code()` and `::hint()` say which), `display` — the shared styled-line |
